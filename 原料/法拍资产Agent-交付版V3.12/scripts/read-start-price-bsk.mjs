...
const missing = state.records.map((record, index) => ({ record, index })).filter(({ record }) => !Number.isFinite(Number(record['起拍价格'])) || Number(record['起拍价格']) <= 0);
...
for (const { record, index } of missing) {
  try {
    ...
    const match = text.match(/cell "(起拍价|起拍价格)\s*:\s*[￥¥]?\s*([\d,.]+)(?:元)?"/);
