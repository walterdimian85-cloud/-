...
JS_EXTRACT = r"""(() => {
  ...
  return JSON.stringify({
    title: (document.title || '').replace(/\s*[-–—]\s*司法拍卖.*$/, ''),
...
def main():
    ...
    for n, (idx, rec) in enumerate(todo, 1):
        ...
        d = ev()
        # 重试一次
        if d is None or (d.get('area') is None and d.get('assess') is None):
            time.sleep(6)
            d = ev()
        if not d:
        ...
        print(f"    -> 起拍={rec.get('起拍价格')} 面积={rec.get('面积/㎡')} 评估={rec.get('评估价')} 法院={rec.get('处置法院')} | 剩余缺{left if left else '无'}", flush=True)
    print(f"\n完成！修复 {fixed} 条")
